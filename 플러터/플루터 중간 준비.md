...
```dart
.idea : 개발 도구에 필요한 설정
android : 안드로이드 네이티브 코드를 작성하는 붑ㄴ
build : 빌드시 생성되는 파일
...
`Radio와 RadioListTile` : 선택 그룹 중 하나를 선택할 때 사용하는 위젯 / 어디까지를 터치 영역으로 볼 것이냐에 따라 Radio를 사용하거나 RadioListTile을 사용하는 방법
...
`SliverAppBar와 SliberList`: AppBar와 List 부분에 스크롤 애니메이션 기능을 주고 싶을때 사용
